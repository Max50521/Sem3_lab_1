...
namespace Sorts_algorithms
{
    ...
    template<class T>
    Sequence<T> *SelectionSort(Sequence<T> *seq)   //Сортировка с помощью простого выбора
    {
    ...
    template<class T>
    Sequence<T> *QuickSort(Sequence<T> *seq)    //быстрая сортировка
    {
        if (seq->GetSize() <= 1)
        {
            throw out_of_range(INCORRECT_INDEX);
    ...
    template<class T>
    Sequence<T> *ShellSort(Sequence<T> *seq)   // сортировка Шелла
    {
        int size = seq->GetSize();
        int d = size / 2;
        T temp;
